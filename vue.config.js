...
console.log(`=====================-=====================`);
console.log(`${packageInfo.name}_v${packageInfo.version}_${outputDirData}_${currentDate}`);
  ...
  pluginOptions: {
    electronBuilder: {
      builderOptions: {
        ...
        "productName": `${packageInfo.name}`, // 项目名，也是生成的安装文件名，即aDemo.exe
        ...
        "win": { //win相关配置
          "icon": "./logo.png", //图标，当前图标在根目录下，注意这里有两个坑
					"artifactName": `${packageInfo.name}.${suffix}`,
          "target": [
            {
        ...
        "nsis": {
          ...
          "shortcutName": `${packageInfo.name}`, // 图标名称
